...
```{python ans12}
index_3ticks = [i for i in range(len(filelines)) if filelines[i].split("{")[0] == "```"]
...
end = [index_3ticks[i]-1 for i in range(len(index_3ticks)) if i%2!=0]
# chunk_start's position
position = [index_3ticks[i] for i in range(len(index_3ticks)) if i%2==0]
language = [chunk_lang(filelines[i]) for i in (position[j] for j in range(len(position)))]
...
```{python ans14}
ending = [index_3ticks[i] for i in range(len(index_3ticks)) if i%2!=0]

python_code_list= list()
for i in range(1,len(ending)+1):
    python_code_list += filelines[dict_chunks['start'][i]:ending[i]]
...
def search_by_uid(uid):
    x = [i for i in range(len(allUIDs)) if allUIDs[i]==uid][0]
    return danceInfo[x]['showInfo'][0]
...
```{python ans23}
updated_result = [search_by_uid('5d35ec95d083a398fcdc5e14')] + [{'time': '2019/10/30 15:00:00', 'location': '新北市三峽區大學路151號', 'locationName': '國立臺北大學'}]
...
for i in range(1,len(M)):
    M_partial += [[M[i][j] for j in range(2,len(M[i]))]]
